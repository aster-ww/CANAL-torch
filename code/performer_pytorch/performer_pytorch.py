    ...
    def forward(self, q, k, v, output_attentions = False):
        ...
        if output_attentions:
            ...
            for head_dim in range(q.shape[1]):##10
                ...
                # print(torch.isnan(attn_weights_0))
...
class Gene2VecPositionalEmbedding(nn.Module):##(16906, 200)
    def __init__(self, dim, max_seq_len, highly_gene_idx):
        super().__init__()
        gene2vec_weight = np.load('./data/gene2vec_16906.npy')
        gene2vec_weight = gene2vec_weight[highly_gene_idx]
        gene2vec_weight = np.concatenate((gene2vec_weight, np.zeros((1, gene2vec_weight.shape[1]))), axis=0)
        gene2vec_weight = torch.from_numpy(gene2vec_weight)##16907*200
        # print("gene2vec_weight:", gene2vec_weight[:10], gene2vec_weight.size())
        self.emb = nn.Embedding.from_pretrained(gene2vec_weight)

    def forward(self, x):
        t = torch.arange(x.shape[1], device=x.device)##16906
        # print("t:",t.size())
        return self.emb(t)
...
class PerformerLM(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.token_emb = nn.Embedding(num_tokens, dim)##一共七个词（状态？），每个词用一个200维的向量来表示
        ##输入：b*seq_len 输出：b*seq_len*200

        if g2v_position_emb:
            self.pos_emb = Gene2VecPositionalEmbedding(dim, max_seq_len, highly_gene_idx)
    ...
    def forward(self, x, return_encodings = False, output_attentions = False, **kwargs):
        ...
        x = self.token_emb(x) # batch*16906*200
        if output_attentions:
            x.requires_grad_()    # used for attn_map output
        x += self.pos_emb(x) # batch*16906*200
        x = self.dropout(x)
